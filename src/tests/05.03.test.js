/**
 * Exercise 05 extra 3 - Mocking HTTP requests with MSW
 ...
 * - Uses snapshot testing to check the error message is displayed
...
test("omitting the password results in an error", async () => {
  ...
  expect(screen.getByRole("alert").textContent).toMatchInlineSnapshot(
    // check the error message is displayed using snapshot testing
    `"password required"`
  );
});
